...
@section('title','LONG VACATION REPORT')
@section('content')
                  ...
                  <table class="table table-bordered">
                    <thead>
                     ...
                     echo  $set['rpt'][0],
                      $set['carry'][0];
                      ?>
                  		<th class="text-center" colspan="1">FIRST SEMESTER RESULTS</th>
                  		<th class="text-center" colspan="1">SECOND SEMESTER RESULTS</th>
...
$elective_grade2 = $R->fetch_electives($v->id,$s,$l,2,$season,$electiveSql2);
 ...
 $repeat_course =$R->repeat_course($v->id,$s,$l,$season,$s,$passedVacationCourseId,$f,$fos);
 //$repeat_course =$R->repeat_course($v->id,$s,$l,$season,$student_reg->session,$passedVacationCourseId);
 $cgpa =$R->get_cgpa($s,$v->id,$season);
 $remark =$R->result_check_pass_sessional($l,$v->id,$s,$cgpa,$season,$fos,'',$failResult['sum_of_F'],$courseWithResult,
 $rccnirq,$totalElective,$passedVacationCourseId);
...
echo '<td class="s9">',
$R->getDropCourse($v->id,$l,$s,$fos,$season,$f),'</td>';
