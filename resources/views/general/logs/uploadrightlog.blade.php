...
<p class="text-center" style="font-size:18px; font-weight:700;">UNIVERSITY OF CALABAR, CALABAR</p>
    ...
    <p class="text-center" style="font-size:14px; font-weight:700;">Upload Right Log</p>
...
<?php   //$collectionLevel =$allcourse->groupBy('level_id')->toArray(); 
           ...
           @foreach($u as $k => $v)  
             <?php 
             $d='';
             $user =$r->getUser($v->user_id);
            if($user != null && $user->department_id != $v->department_id){
             $d =$r->get_departmetname($user->department_id);
            }
             $dlog =$r->get_departmetname($v->department_id); ?>
                <tr>
                      <tr>
                      <td>{{++$c}}</td>
